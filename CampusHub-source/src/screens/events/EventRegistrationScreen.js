  ...
  const handleRegister = async () => {
    ...
    try {
      const registrationData = {
        eventId: event.id,
        userId: user?.id, // Send user ID if available
        name,
        email,
        additionalInfo,
      };

      // Replace with your actual API endpoint for event registration
      const response = await apiClient.post('/api/events/register', registrationData);
      console.log('Event registration successful:', response.data);

      setSnackbarMessage('Successfully registered for the event!');
      setSnackbarVisible(true);
      setTimeout(() => {
        navigation.goBack(); // Go back to event details or events list
      }, 2000);
